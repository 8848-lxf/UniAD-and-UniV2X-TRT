#!/usr/bin/env python3
"""Run a UniAD TensorRT-controlled closed loop on an existing CARLA route."""
...
CAMERAS = (
    ("front", 1.5, 0.0, 1.8, 0.0),
    ("front_right", 1.2, 0.0, 1.8, 60.0),
    ("front_left", 1.2, 0.0, 1.8, -60.0),
    ("back", -1.5, 0.0, 1.8, 180.0),
    ("back_left", -1.2, 0.0, 1.8, -120.0),
    ("back_right", -1.2, 0.0, 1.8, 120.0),
)
...
def parse_args():
    ...
    parser.add_argument("--traffic-vehicles", type=int, default=0)
...
def model_control(carla, planning, speed_mps, args):
    ...
    # UniAD lateral x and CARLA steering are both positive to the right.
...
def main():
    ...
    try:
        ...
        road_start = world.get_map().get_waypoint(
            requested_start.location, project_to_road=True, lane_type=carla.LaneType.Driving
        )
        candidates = []
        if road_start is not None:
            candidates.append(road_start.transform)
            candidates.extend(item.transform for item in road_start.previous(2.0))
            candidates.extend(item.transform for item in road_start.next(2.0))
        candidates.append(requested_start)
        ego = None
        ...
        for candidate in candidates:
            for height_offset in (0.5, 1.5, 2.5):
                attempted = carla.Transform(
                    carla.Location(
                        x=candidate.location.x, y=candidate.location.y,
                        z=candidate.location.z + height_offset,
                    ),
                    requested_start.rotation,
                )
        ...
        for name, x, y, z, yaw in CAMERAS:
            ...
            camera_actors.append((name, camera))
            camera_queues.append(sensor_queue)
        ...
        for frame_index in range(args.max_frames):
            ...
            if frame_index % args.inference_interval == 0:
                images = []
                camera_transforms = []
                for (name, camera), sensor_queue in zip(camera_actors, camera_queues):
                    images.append(camera_array(matching_measurement(
                        sensor_queue, carla_frame, name + " camera"
                    )))
                    camera_transforms.append(transform_matrix(camera.get_transform()))
                ...
                np.savez(
                    ...
                    world_from_ego=world_from_ego,
                    world_from_cameras=np.stack(camera_transforms),
                    ...
                    camera_width=np.asarray([1600], dtype=np.int32),
                    camera_height=np.asarray([900], dtype=np.int32),
        ...
        result = {
            ...
            "latency": {
                "steady_state_excluding_first_inference": {
                    ...
                    for key in ("engine_forward", "service_end_to_end")
                },
