...
def parse_args():
    ...
    parser.add_argument(
        '--onnx-output',
        default='./onnx/uniad_tiny_imgx0.25_cp.onnx',
        help='path for the initial ONNX export')
    parser.add_argument(
        '--onnx-input-dir',
        default='./nuscenes_np/uniad_onnx_input',
        help='directory containing the six exported ONNX inputs')
    parser.add_argument(
        '--onnx-runtime-output-dir',
        default='./nuscenes_np/uniad_pth_trtp_out',
        help='directory for PyTorch temporal outputs used during tracing')
    parser.add_argument(
        '--raw-input-dump-dir',
        default='./dumped_inputs',
        help='directory for raw input tensors used by external validation')
...
def main():
    ...
    # ########## ONNX Export Starts###############
    onnx_file_name = args.onnx_output
    onnx_folder = os.path.dirname(os.path.abspath(onnx_file_name)) + '/'
    folder_dat = os.path.abspath(args.raw_input_dump_dir) + '/'
    onnx_export_input = os.path.abspath(args.onnx_input_dir) + '/'
    onnx_export_output = os.path.abspath(args.onnx_runtime_output_dir) + '/'
