...
class UniV2XTrack(MVXTwoStageDetector):
    ...
    def get_bevs(self, imgs, img_metas, prev_img=None, prev_img_metas=None, prev_bev=None, meta_tensors=None):
        ...
        if self.freeze_bev_encoder:
            with torch.no_grad():
                bev_embed, bev_pos = self.pts_bbox_head.get_bev_features(
                    mlvl_feats=img_feats, img_metas=img_metas, prev_bev=prev_bev, meta_tensors=meta_tensors)
        else:
            bev_embed, bev_pos = self.pts_bbox_head.get_bev_features(
                    mlvl_feats=img_feats, img_metas=img_metas, prev_bev=prev_bev, meta_tensors=meta_tensors)
    ...
    def _forward_single_frame_inference(
        ...
        # NOTE: You can replace BEVFormer with other BEV encoder and provide bev_embed here
        bev_embed, bev_pos = self.get_bevs(
            img, img_metas, prev_bev=prev_bev, meta_tensors=meta_tensors
        )
    ...
    def simple_test_track(
        ...
        if (
            isinstance(meta_tensors, dict)
            and "is_new_scene" in meta_tensors
            and meta_tensors["is_new_scene"] is not None
        ):
            flag = meta_tensors["is_new_scene"]
            if isinstance(flag, torch.Tensor):
                is_new_scene = bool(flag.reshape(-1)[0].detach().cpu().item())
            else:
                is_new_scene = bool(flag)
        else:
            prev_scene_token = getattr(self, "scene_token", None)
            is_new_scene = (img_metas[0]["scene_token"] != prev_scene_token)
