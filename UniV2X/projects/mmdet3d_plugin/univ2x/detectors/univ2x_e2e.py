...
class UniV2X(UniV2XTrack):
    ...
    def forward_test(self,
        ...
        trace_meta = dict(meta_tensors) if isinstance(meta_tensors, dict) else None
        if trace_meta is not None and isinstance(trace_meta.get("can_bus", None), torch.Tensor):
            if "is_new_scene" in trace_meta and trace_meta["is_new_scene"] is not None:
                flag = trace_meta["is_new_scene"]
                if isinstance(flag, torch.Tensor):
                    is_new_scene = bool(flag.reshape(-1)[0].detach().cpu().item())
                else:
                    is_new_scene = bool(flag)
            else:
                is_new_scene = (
                    img_metas[0][0]['scene_token'] != self.prev_frame_info['scene_token']
                )
                trace_meta["is_new_scene"] = trace_meta["can_bus"].new_tensor(
                    [1 if is_new_scene else 0]
                )
            ...
            can_bus_now = trace_meta["can_bus"]
            can_bus_for_model = can_bus_now.clone()
            if is_new_scene:
                can_bus_for_model[..., :3] = 0
                can_bus_for_model[..., -1] = 0
            else:
                prev_pos = self.prev_frame_info['prev_pos']
                prev_angle = self.prev_frame_info['prev_angle']
                can_bus_for_model[..., :3] = can_bus_for_model[..., :3] - can_bus_for_model.new_tensor(prev_pos)
                can_bus_for_model[..., -1] = can_bus_for_model[..., -1] - can_bus_for_model.new_tensor(prev_angle)

            self.prev_frame_info['prev_pos'] = can_bus_now[..., :3].detach().reshape(-1, 3)[0].cpu().numpy()
            self.prev_frame_info['prev_angle'] = float(
                can_bus_now[..., -1].reshape(-1)[0].detach().cpu().item()
            )
            trace_meta["can_bus"] = can_bus_for_model
        else:
