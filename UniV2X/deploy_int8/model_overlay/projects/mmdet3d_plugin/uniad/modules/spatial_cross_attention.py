...
class MSDeformableAttention3DTRTP(MSDeformableAttention3DTRT):
    ...
    def _forward_grid_sample(
        ...
        batch, num_query = reference_points.shape[:2]
        points_per_group = reference_points.shape[-1] // 2
        points_per_level = sampling_offsets.shape[-1] // (self.num_levels * 2)
        points_per_reference = points_per_level // points_per_group
        offsets = sampling_offsets.view(
            batch,
            num_query,
            self.num_heads,
            self.num_levels,
        ...
        locations = reference_points.view(
            batch, num_query, 1, 1, 1, points_per_group, 2
        ) + offsets / normalizer.view(1, 1, 1, self.num_levels, 1, 1, 2)
        locations = locations.reshape(
            batch,
            num_query,
            self.num_heads,
            self.num_levels,
        ...
        weights = attention_weights.view(
            batch, num_query, self.num_heads, self.num_levels * points_per_level
        ).softmax(-1).view(
            batch,
            num_query,
            self.num_heads,
        ...
        channel = self.embed_dims // self.num_heads
        level_values = []
        start = 0
        for level in range(self.num_levels):
            ...
            count = height * width
            level_values.append(
                value[:, start:start + count].permute(0, 2, 3, 1).reshape(
                    batch * self.num_heads, channel, height, width
                )
            )
            start += count
        ...
        query_chunk = 4096
        output_chunks = []
        for query_start in range(0, num_query, query_chunk):
            query_stop = min(query_start + query_chunk, num_query)
            chunk_size = query_stop - query_start
            chunk_output = value.new_zeros(
                batch, chunk_size, self.num_heads, channel
            )
            for level in range(self.num_levels):
                grid = locations[:, query_start:query_stop, :, level]
                grid = grid.permute(0, 2, 1, 3, 4).reshape(
                    batch * self.num_heads, chunk_size, points_per_level, 2
                ) * 2.0 - 1.0
                ...
                level_weights = weights[
                    :, query_start:query_stop, :, level
                ].permute(0, 2, 1, 3).reshape(
                    batch * self.num_heads, 1, chunk_size, points_per_level
                )
                level_output = (sampled * level_weights).sum(-1)
                level_output = level_output.view(
                    batch, self.num_heads, channel, chunk_size
                ).permute(0, 3, 1, 2)
                chunk_output = chunk_output + level_output
    ...
    def forward_trt(
        ...
        **kwargs,
        ...
        if getattr(self, "use_native_grid_sample", False):
            output = self._forward_grid_sample(
                value,
                spatial_shapes,
                reference_points,
                sampling_offsets,
