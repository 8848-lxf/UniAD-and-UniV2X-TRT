...
class PerceptionTransformerUniADTRTP(PerceptionTransformerUniADTRT):
    ...
    def get_bev_features_trt(
        ...
        for lvl, feat in enumerate(mlvl_feats):
            ...
            feat = feat.flatten(3).permute(1, 0, 3, 2)
            # UniV2X supplies one image per agent while the inherited UniAD
            # encoder keeps six projection slots. TensorRT 10.9 did not retain
            # the implicit 1-to-6 broadcast here, so materialize the copies.
            feat = feat.repeat(6, 1, 1, 1)
            if self.use_cams_embeds:
