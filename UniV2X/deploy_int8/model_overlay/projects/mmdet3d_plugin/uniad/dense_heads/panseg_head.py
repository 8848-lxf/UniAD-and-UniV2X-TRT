...
class PansegformerHeadTRT(PansegformerHead):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # The map head always uses a fixed all-valid 200x200 BEV mask. Exporting
        # the legacy cumsum/floor-div/sin/cos positional graph changes its
        # semantics in TensorRT 10.9, so retain the exact PyTorch values as a
        # non-checkpoint buffer instead.
        static_mask = torch.zeros(
            1, self.bev_h, self.bev_w, dtype=torch.bool
        )
        with torch.no_grad():
            static_position = self.positional_encoding(static_mask)
        position_channels = static_position.shape[1] // 2
        self.register_buffer(
            "_trt_static_position_y",
            static_position[:, :position_channels, :, :1],
            persistent=False,
        )
        self.register_buffer(
            "_trt_static_position_x",
            static_position[:, position_channels:, :1, :],
            persistent=False,
        )
    ...
    def forward_trt(self,
        ...
        for feat in mlvl_feats:
            ...
            static_y = self._trt_static_position_y.to(feat)
            static_x = self._trt_static_position_x.to(feat)
            if (static_y.shape[-2] != feat.shape[-2]
                    or static_x.shape[-1] != feat.shape[-1]):
                raise ValueError(
                    "Static map position encoding shape does not match BEV"
                )
            mlvl_positional_encodings.append(
                torch.cat(
                    [
                        static_y.expand(
                            feat.shape[0], -1, -1, feat.shape[-1]
                        ),
                        static_x.expand(
                            feat.shape[0], -1, feat.shape[-2], -1
                        ),
                    ],
                    dim=1,
                )
    ...
    @staticmethod
    def _scatter_lane_bev_trt(bev, query, reference, projection):
        height = width = 200
        xy = (reference[:, :2].sigmoid() * reference.new_tensor(
            [width, height]
        )).long()
        axis = torch.arange(-2, 2, device=xy.device)
        offsets = torch.stack(
            [
        ...
        updates = projection(query)[:, None].expand(
            -1, offsets.shape[0], -1
        )[valid]
    ...
    def forward_test_trt(self,
                    ...
                    other_outputs_classes=None,
                    other_outputs_coords=None,
                    other_query=None,
                    other_query_pos=None,
        ...
        if other_outputs_classes is not None:
            (
                outputs_classes,
                outputs_coords,
                query,
                ...
                added_query_pos,
                added_reference,
            ) = self.cross_lane_fusion.forward_trt(
            ...
            memory = self._scatter_lane_bev_trt(
                memory,
                added_query[0],
                added_reference[0],
                self.bev_embed_linear,
            )
            memory_pos = self._scatter_lane_bev_trt(
                memory_pos,
                added_query_pos[0],
                added_reference[0],
                self.bev_pos_linear,
        ...
        map_raw_masks,\
        map_raw_scores,\
        map_raw_labels = self.get_bboxes_trt(outputs_classes,
                        outputs_coords,
                        memory,
                        memory_mask,
                        query,
