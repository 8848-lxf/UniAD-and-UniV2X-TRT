...
class OccHeadTRT(OccHead):
    ...
    def forward_trt(self, x, ins_query):
        ...
        if hasattr(self, "pred_prob"):
            probs = []
            for ti in range(future_states.size(1)):
                probs.append(self.pred_prob[ti](future_states[:, ti]))
            ins_occ_logits = torch.stack(probs, dim=1).permute(0, 2, 1, 3, 4)
        else:
            ins_occ_query = self.query_to_occ_feat(ins_query)
            ins_occ_logits = torch.einsum(
                "btqc,btchw->bqthw", ins_occ_query, future_states
            )
    ...
    def forward_test_trt(
        ...
        if not hasattr(self, "pred_prob"):
            # The original UniV2X test path does not confidence-scale valid
            # occupancy queries. Use scores only as a fixed-slot validity mask
            # so TensorRT padding cannot create foreground instances.
            valid_queries = (track_scores > 0).to(pred_ins_sigmoid)
            pred_ins_sigmoid = pred_ins_sigmoid * valid_queries[:, :, None, None, None]

        pred_seg_scores2 = pred_ins_sigmoid.max(1)[0]
        if other_occ_scores is not None:
            pred_seg_scores2 = self.occ_prob_fusion_trt(
                pred_seg_scores2, other_occ_scores, ego2other_rt
            )
        seg_out = (pred_seg_scores2 > self.test_seg_thresh).int()[:,:,None,...]  # [b, t, 1, h, w]
        # seg_out = (seg_out*(track_query_shape1!=0)).long()

        return seg_gt, seg_out, pred_seg_scores2, pred_ins_sigmoid

    def occ_prob_fusion_trt(self, veh_occ, inf_occ, ego2other_rt):
        batch_size = veh_occ.shape[0]
        bev_h, bev_w = self.bev_size
        xs = torch.linspace(
            0.5, bev_w - 0.5, bev_w,
            dtype=veh_occ.dtype, device=veh_occ.device
        ).view(1, bev_w).expand(bev_h, bev_w) / bev_w
        ys = torch.linspace(
            0.5, bev_h - 0.5, bev_h,
            dtype=veh_occ.dtype, device=veh_occ.device
        ).view(bev_h, 1).expand(bev_h, bev_w) / bev_h
        grid = torch.stack((xs, ys), dim=-1)[None].expand(
            batch_size, -1, -1, -1
        )
        pc_range = self.univ2x_pc_range
        grid_x = grid[..., 0:1] * (pc_range[3] - pc_range[0]) + pc_range[0]
        grid_y = grid[..., 1:2] * (pc_range[4] - pc_range[1]) + pc_range[1]
        grid = torch.cat([grid_x, grid_y, torch.ones_like(grid_x)], dim=-1)
        transform = ego2other_rt[:, [0, 1, 3]][:, :, [0, 1, 3]]
        grid = torch.matmul(grid, transform)[..., :2]
        inf_range = self.univ2x_inf_pc_range
        grid_x = 2.0 * (grid[..., 0:1] - inf_range[0]) / (
            inf_range[3] - inf_range[0]
        ) - 1.0
        grid_y = 2.0 * (grid[..., 1:2] - inf_range[1]) / (
            inf_range[4] - inf_range[1]
        ) - 1.0
        sample_grid = torch.cat([grid_x, grid_y], dim=-1)
        aligned_inf_occ = F.grid_sample(
            inf_occ, sample_grid, align_corners=True
        )
        veh_binary = (veh_occ > self.test_seg_thresh).to(veh_occ.dtype)
        inf_binary = (aligned_inf_occ > self.test_seg_thresh).to(veh_occ.dtype)
        return torch.maximum(veh_binary, inf_binary)
