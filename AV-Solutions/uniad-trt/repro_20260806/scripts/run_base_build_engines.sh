...
MIN=${TRACK_PROFILE_MIN:-901}
OPT=${TRACK_PROFILE_OPT:-901}
MAX=${TRACK_PROFILE_MAX:-1400}
...
if [[ ",${ENGINE_SET}," == *,fp32,* ]]; then
  ...
  build_engine "${FP_ONNX}" "${ENGINE_DIR}/uniad_base_fp32.engine" "${CACHE_DIR}/fp32.cache" "${FP32_FLAGS[@]}"
fi
