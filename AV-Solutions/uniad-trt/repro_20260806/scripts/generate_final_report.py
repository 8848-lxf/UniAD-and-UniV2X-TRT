...
def row_from_pytorch():
    ...
    return {
        ...
        "model_p99_ms": latency["model_forward"].get("p99_ms"),
        ...
        "e2e_p99_ms": latency["end_to_end"].get("p99_ms"),
...
def row_from_trt(precision, label):
    ...
    return {
        ...
        "model_p99_ms": latency["model_enqueue"]["p99_ms"],
        "fps_from_mean": latency["model_enqueue"]["fps_from_mean"],
        "e2e_mean_ms": latency["end_to_end"]["mean_ms"],
        "e2e_p50_ms": latency["end_to_end"]["p50_ms"],
        "e2e_p99_ms": latency["end_to_end"]["p99_ms"],
        "avg_l2_m": planning["avg_l2_m"],
        "avg_col_percent": planning["avg_box_collision_percent"],
        "avg_point_col_percent": planning["avg_point_collision_percent"],
        "planning_reference_avg_l2_m": planning.get(
            "planning_mse", planning["planning_reference_avg_l2_m"]
        ),
...
def main():
    ...
    lines += [
        ...
        "",
        "| Precision | Model mean (ms) | Model p50 (ms) | Model p99 (ms) | FPS (mean) | E2E mean (ms) | E2E p50 (ms) | E2E p99 (ms) | avg. L2 (m) | avg. Col (%) | planning MSE* |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for key, value in local.items():
        lines.append(
            f"| {value['label']} | {fmt(value['model_mean_ms'])} | {fmt(value['model_p50_ms'])} | "
            f"{fmt(value['model_p99_ms']) if value['model_p99_ms'] is not None else 'n/a'} | "
            f"{fmt(value['fps_from_mean'], 2)} | {fmt(value['e2e_mean_ms'])} | "
            f"{fmt(value['e2e_p50_ms'])} | "
            f"{fmt(value['e2e_p99_ms']) if value['e2e_p99_ms'] is not None else 'n/a'} | "
            f"{fmt(value['avg_l2_m'])} | "
    ...
    lines += [
        ...
        "",
        "`planning MSE*` follows NVIDIA's table semantics: mean Euclidean L2 distance in meters between TensorRT and PyTorch trajectories. The JSON also reports the literal coordinate-wise squared-error mean as `planning_reference_coordinate_mse`.",
        "",
        "## Artifact hashes",
        "",
