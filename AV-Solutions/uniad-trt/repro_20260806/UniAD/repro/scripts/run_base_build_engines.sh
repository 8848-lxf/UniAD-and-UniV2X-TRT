...
MIN=901
OPT=901
# The base graph emits up to 1400 temporal track rows on this checkpoint.
# The tiny tutorial uses 1150; reusing that cap truncates a valid base frame.
MAX=1400
...
if [[ ",${ENGINE_SET}," == *,fp32,* ]]; then
  build_engine "${FP_ONNX}" "${ENGINE_DIR}/uniad_base_fp32${ENGINE_SUFFIX}.engine" "${CACHE_DIR}/fp32${ENGINE_SUFFIX}.cache"
fi
if [[ ",${ENGINE_SET}," == *,fp16,* ]]; then
  build_engine "${FP_ONNX}" "${ENGINE_DIR}/uniad_base_fp16${ENGINE_SUFFIX}.engine" "${CACHE_DIR}/fp16${ENGINE_SUFFIX}.cache" --fp16
fi
if [[ ",${ENGINE_SET}," == *,int8_eq_fp16,* ]]; then
  build_engine "${INT8_ONNX}" "${ENGINE_DIR}/uniad_base_int8_eq_fp16${ENGINE_SUFFIX}.engine" "${CACHE_DIR}/int8_eq_fp16${ENGINE_SUFFIX}.cache" --best
fi
