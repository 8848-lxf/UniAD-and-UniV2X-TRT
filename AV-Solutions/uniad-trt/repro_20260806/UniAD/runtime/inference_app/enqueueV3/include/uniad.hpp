...
#define TRACK_INS_MAX 2200
#define TRACK_MAX 1000
#define TRACK_INS_MIN 901

#ifndef UNIAD_BEV_H
#define UNIAD_BEV_H 50
#endif
#ifndef UNIAD_BEV_W
#define UNIAD_BEV_W 50
#endif
#ifndef UNIAD_IMG_H
#define UNIAD_IMG_H 256
#endif
#ifndef UNIAD_IMG_W
#define UNIAD_IMG_W 416
#endif

#define UNIAD_BEV_SIZE (UNIAD_BEV_H * UNIAD_BEV_W)
#define UNIAD_AUDIT_MAX_ELEMENTS (5 * 256 * 52 * 52)
...
struct KernelParams {
  ...
  std::unordered_map<std::string, std::vector<TRT_INT_TYPE>> input_max_shapes = {
    ...
    {"prev_l2g_t", {1, 3}},
    {"prev_bev", {UNIAD_BEV_SIZE, 1, 256}},
    {"timestamp", {1}},
    {"l2g_r_mat", {1, 3, 3}},
    {"l2g_t", {1, 3}},
    {"img", {1, 6, 3, UNIAD_IMG_H, UNIAD_IMG_W}},
  ...
  std::unordered_map<std::string, std::vector<TRT_INT_TYPE>> output_max_shapes = {
    ...
    {"prev_l2g_t_out", {1, 3}},
    {"bev_embed", {UNIAD_BEV_SIZE, 1, 256}},
    ...
    {"audit_out", {UNIAD_AUDIT_MAX_ELEMENTS}}
  };
};
...
struct KernelInput {
  ...
  std::vector<float> prev_bev = std::vector<float>(UNIAD_BEV_SIZE*1*256, 0);
  ...
  std::vector<float> img = std::vector<float>(1*6*3*UNIAD_IMG_H*UNIAD_IMG_W);
...
struct KernelOutput {
  ...
  std::vector<float> bev_embed = std::vector<float>(UNIAD_BEV_SIZE*1*256);
  ...
  std::vector<float> audit_out = std::vector<float>(UNIAD_AUDIT_MAX_ELEMENTS);
...
class Kernel {
 public:
  ...
  virtual float last_model_latency_ms() const = 0;
...
public:
  ...
  virtual float last_model_latency_ms() const;
...
private:
  ...
  std::vector<size_t> output_capacity_bytes_;
};
