...
def main():
    ...
    parser.add_argument("--contract", choices=("bev", "map"), default="bev")
    ...
    spatial_shape_values = (
        [[200, 200]] if args.contract == "map"
        else [[136, 240], [68, 120], [34, 60], [17, 30]]
    )
    spatial_shapes = torch.tensor(
        spatial_shape_values, dtype=torch.int64, device=device
    )
    spatial_size = int((spatial_shapes[:, 0] * spatial_shapes[:, 1]).sum())
    level_count = int(spatial_shapes.shape[0])
    inputs = (
        torch.randn(args.batch_size, spatial_size, 8, 32, device=device),
        spatial_shapes,
        torch.rand(args.batch_size, args.query_count, level_count, 2, device=device),
        torch.randn(
            args.batch_size, args.query_count, 8, level_count * 4 * 2,
            device=device,
        ),
        torch.randn(
            args.batch_size, args.query_count, 8, level_count * 4,
            device=device,
        ),
    )
