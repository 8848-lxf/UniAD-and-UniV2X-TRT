...
def _compact_mismatched_attention_levels(
    sampling_offsets,
    attention_weights,
    reference_points,
    spatial_shapes,
    ...
    """Reproduce MMCV CUDA's flat read when runtime levels are fewer."""
    runtime_num_levels = int(spatial_shapes.size(0))
    batch_size, num_queries = attention_weights.shape[:2]
    active_values = (
        num_queries * num_heads * runtime_num_levels * num_points
    )

    normalized_weights = attention_weights.flatten(-2).softmax(-1)
    normalized_weights = normalized_weights.view_as(attention_weights)
    compact_normalized = normalized_weights.flatten(1)[..., :active_values].view(
        batch_size,
        num_queries,
        num_heads,
        runtime_num_levels,
    ...
    active_weight_mass = compact_normalized.sum(dim=(-2, -1))
    compact_weights = attention_weights.flatten(1)[..., :active_values].view(
        batch_size,
        num_queries,
        num_heads,
        runtime_num_levels,
    ...
    if reference_points.shape[-1] == 2:
        ...
        raise ValueError(
            "Reference points must have a final dimension of 2 or 4"
        )

    compact_locations = sampling_locations.flatten(1)[
        ..., :active_values * 2
    ].view(
        batch_size,
        num_queries,
    ...
    runtime_normalizer = torch.stack(
        [spatial_shapes[..., 1], spatial_shapes[..., 0]], dim=-1
    )
    compact_offsets = (
        compact_locations
        * runtime_normalizer[None, None, None, :, None, :]
    )
    compact_reference = reference_points.new_zeros(
        (batch_size, num_queries, runtime_num_levels, 2)
    )
    return (
        compact_offsets,
        compact_weights,
...
class CustomMSDeformableAttentionTRTP(CustomMSDeformableAttentionTRT):
    ...
    def forward_trt(
        ...
        **kwargs,
        ...
        # Pansegformer supplies one BEV level although the original UniV2X
        # map-attention projections were trained with num_levels=4. Reproduce
        # the flat-memory behavior of MMCV's CUDA kernel before invoking the
        # TRT plugin; otherwise the plugin interprets all 16 projected points
        # as belonging to the single runtime level.
        runtime_num_levels = int(spatial_shapes.size(0))
        sampling_offsets = sampling_offsets.view(
            *sampling_offsets.shape[:2],
            self.num_heads,
            self.num_levels,
            self.num_points,
            2,
        ...
        if runtime_num_levels != self.num_levels:
            if self.num_levels % runtime_num_levels != 0:
                raise ValueError(
                    "Configured attention levels must be divisible by runtime levels"
                )
            (
                sampling_offsets,
                attention_weights,
                reference_points,
                active_weight_mass,
            ) = _compact_mismatched_attention_levels(
                sampling_offsets,
                attention_weights,
                reference_points,
                spatial_shapes,
            ...
            active_weight_mass = None

        sampling_offsets = sampling_offsets.flatten(-3).contiguous()
        attention_weights = attention_weights.flatten(-2).contiguous()
        ...
        if active_weight_mass is not None:
            output = output.view(
                *output.shape[:2], self.num_heads, self.embed_dims // self.num_heads
            )
            output = (output * active_weight_mass.unsqueeze(-1)).flatten(2)
