...
class UniADTrackTRT(UniADTrack):
    ...
    def _forward_single_frame_inference_preprocess_trt_v2(
        ...
        # UniV2X preserves the stored reference point and applies motion
        # compensation only to x/y. Recomputing z from the query changes the
        # temporal detector state and eventually drops otherwise valid tracks.
    ...
    def _forward_single_frame_inference_trt(
        ...
        if coop_track_instances is not None:
            track_instances, added_query, added_reference = \
                self.fuse_agent_tracks_trt(
            ...
            query = track_instances[0]
            ref_pts = track_instances[1]
            bev_embed, bev_pos = self.augment_track_bev_trt(
