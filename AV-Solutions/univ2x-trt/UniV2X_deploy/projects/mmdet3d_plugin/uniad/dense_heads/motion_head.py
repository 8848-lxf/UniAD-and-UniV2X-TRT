...
class MotionHeadTRT(MotionHead):
    ...
    def forward_test_trt(self,
        ...
        labels[-1] = 0
        # The fixed-capacity export pads object slots with label -1. Expose the
        # real validity mask while preserving the static TensorRT output shape.
        valid_traj_masks = (labels >= 0)[None, :].to(valid_traj_masks.dtype)
...
class MotionHeadTRTP(MotionHeadTRT):
    ...
    def forward_test_trt(self,
        ...
        track_boxes_2 = torch.cat([track_boxes_2,
                                   sdc_track_boxes_2], dim=0)
        track_boxes_2[-1] = 0
        agent_key_padding_mask = (track_boxes_2 < 0)[None, :]
        ...
        track_scores = scores[None, :]
        labels[-1] = 0
        # Static export keeps padded object slots in the trajectory tensors.
        # Preserve their shape but exclude them from reported valid trajectories.
        valid_traj_masks = (labels >= 0)[None, :].to(valid_traj_masks.dtype)
