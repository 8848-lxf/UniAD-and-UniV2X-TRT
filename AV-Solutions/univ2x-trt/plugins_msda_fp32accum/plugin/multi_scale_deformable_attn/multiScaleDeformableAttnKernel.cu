...
__device__ float ms_deform_attn_im2col_bilinear_half_float(
    const __half *&bottom_data, const int &height, const int &width,
  ...
  float v1 = 0.0f;
  if (h_low >= 0 && w_low >= 0) {
    v1 = __half2float(bottom_data[(h_low * width + w_low) * step]);
  }
  float v2 = 0.0f;
  if (h_low >= 0 && w_high < width) {
    v2 = __half2float(bottom_data[(h_low * width + w_high) * step]);
  }
  float v3 = 0.0f;
  if (h_high < height && w_low >= 0) {
    v3 = __half2float(bottom_data[(h_high * width + w_low) * step]);
  }
  float v4 = 0.0f;
  if (h_high < height && w_high < width) {
    v4 = __half2float(bottom_data[(h_high * width + w_high) * step]);
  }
...
#pragma unroll
    for (int w_index = 0; w_index < all_points; ++w_index) {
      const float weight = __half2float(data_attn_weight[data_weight_ptr + w_index]);
      max_weight = max(max_weight, weight);
    }
    ...
    for (int level_index = 0; level_index < num_levels; ++level_index) {
      ...
      for (int point_index = 0; point_index < num_point; ++point_index) {
        const int point_index_per_group = point_index % points_per_group;
        const float reference_point_x = __half2float(
            data_reference_points[data_points_ptr + point_index_per_group * 2]);
        const float reference_point_y = __half2float(
            data_reference_points[data_points_ptr + point_index_per_group * 2 + 1]);
        const float loc_w = reference_point_x * spatial_w +
                            __half2float(data_sampling_offsets[data_offset_w_ptr]);
        const float loc_h = reference_point_y * spatial_h + __half2float(
            data_sampling_offsets[data_offset_w_ptr + 1]);

        const float weight = expf(
            __half2float(data_attn_weight[data_weight_ptr]) - max_weight);
        sum_weight += weight;

        const float h_im = loc_h - 0.5f;
        const float w_im = loc_w - 0.5f;

        if (h_im > -1.0f && w_im > -1.0f && h_im < spatial_h &&
            w_im < spatial_w) {
          output += ms_deform_attn_im2col_bilinear_half_float(
    ...
    *data_output_ptr = __float2half_rn(output / sum_weight);
  }
}
